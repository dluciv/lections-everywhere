...
$$b_i \gets b_i + \lambda \frac{\partial (f(x - b_i) - f^{\sim}(x - b_i))}{\partial b_i}$$
...
* Следующий $x \gets x + \lambda (\Delta y) \nabla g$
...
[Приемр с исключающим «или» и импликацией](https://github.com/dluciv/python-intro-course/blob/master/docs/jupiter-notebooks/10.a.neural.dluciv.ipynb)
