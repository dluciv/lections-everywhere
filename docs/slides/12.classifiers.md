...
$$f(x) \approx c \sum\_{i=1}^{N} \sigma(a_i(x + b_i))$$

Надо подобрать $c$ и $a_i$ с $b_i$ в количестве $2N$ штук.

-{pause}-

На самом деле для аппроксимации на отрезке $[0, \pi/2]$ хватит

$$f(x) \approx \frac{1}{N} \sum\_{i=1}^{N} \sigma\left(a\frac{2N}{\pi} (x + b_i)\right)$$

$a$ — константа, чтобы сделать сигмоиду «круче», например $100/\pi$
